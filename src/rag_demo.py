...
def build_corpus(docs: List[Tuple[str, str]], chunk_size=400, overlap=80) -> List[Tuple[str, str, int]]:
    """
...
class VectorIndex:
    ...
    def fit(self, corpus: List[Tuple[str, str, int]]):
        ...
        embs = self.embedder.encode(texts, batch_size=64, show_progress_bar=True, convert_to_numpy=True, normalize_embeddings=True)
        d = embs.shape[1]
    ...
    def query(self, question: str, top_k: int = 4) -> List[Tuple[int, float]]:
        q_emb = self.embedder.encode([question], convert_to_numpy=True, normalize_embeddings=True).astype(np.float32)
...
def build_prompt(question: str, context_chunks: List[str]) -> str:
    # Keep it compact for small models: instruction + context + question
    ...
    # T5 expects a task-like prompt; keep it simple
...
def run_rag(data_folder="data", question="What are common Git commands?", top_k=4):
    ...
    corpus = build_corpus(docs, chunk_size=300, overlap=60)
    ...
    # 4) Generate answer using local T5
    gen = LocalGenerator()
